...
let orchestrator, MockLbdTokenMonetaryPolicy, mockLbdTokenMonetaryPolicy, MockDownstream, mockDownstream
let r
...
async function setupContracts () {
    ...
    MockLbdTokenMonetaryPolicy = await ethers.getContractFactory('MockLbdTokenMonetaryPolicy')
    mockLbdTokenMonetaryPolicy = await MockLbdTokenMonetaryPolicy.deploy()
    await mockLbdTokenMonetaryPolicy.deployed()
    mockLbdTokenMonetaryPolicy = mockLbdTokenMonetaryPolicy.connect(deployer)

    const LbdTokenOrchestrator = await ethers.getContractFactory('LbdTokenOrchestrator')
    orchestrator = await upgrades.deployProxy(LbdTokenOrchestrator, [mockLbdTokenMonetaryPolicy.address])
...
describe('LbdTokenOrchestrator', () => {
    before('setup LbdTokenOrchestrator contract', setupContracts)
    ...
    describe('when rebase called by a contract which is being constructed', () => {
        it('should fail', async () => {
            ...
            // constructorRebaseCallerContract = await upgrades.deployProxy(ConstructorRebaseCallerContract, [mockLbdTokenMonetaryPolicy.address])
    ...
    describe('when transaction list is empty', async () => {
        ...
        it('should call rebase on policy', async () => {
            const fnCalled = MockLbdTokenMonetaryPolicy.interface.decodeEventLog('FunctionCalled', r.events[0].data)
            expect(fnCalled[0]).to.equal('LbdTokenMonetaryPolicy')
