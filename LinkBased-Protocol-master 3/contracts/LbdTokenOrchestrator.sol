...
import "./LbdTokenMonetaryPolicy.sol";


/**
 * @title LbdTokenOrchestrator
 * @notice The orchestrator is the main entry point for rebase operations. It coordinates the policy
 * actions with external consumers.
 */
contract LbdTokenOrchestrator is OwnableUpgradeSafe {
    ...
    LbdTokenMonetaryPolicy public policy;

    function setMonetaryPolicy(address _policy)
        public
        onlyOwner
    {
        policy = LbdTokenMonetaryPolicy(_policy);
    }

    /**
     * @param policy_ Address of the LbdToken policy.
     */
    ...
    {
        __Ownable_init();
        policy = LbdTokenMonetaryPolicy(policy_);
    }

    /**
     * @notice Main entry point to initiate a rebase operation.
     *         The LbdTokenOrchestrator calls rebase on the policy and notifies downstream applications.
