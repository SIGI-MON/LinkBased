...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    uint private _rate; // 200000000

    uint private _minPurchaseAmount = 200000000;

    // Amount of wei raised
    uint private _weiRaised;

    // Max purchase per user
    uint public maxWeiPerUser = 7 * 10e18;

    // Accounting on information how much user spent.
    mapping (address => uint) public totalWeiSpent;
    ...
    payable
    {
        ...
        require(weiAmount >= _minPurchaseAmount);
        // Add max contribution per wallet
        require(totalWeiSpent[beneficiary].add(weiAmount) <= maxWeiPerUser);
